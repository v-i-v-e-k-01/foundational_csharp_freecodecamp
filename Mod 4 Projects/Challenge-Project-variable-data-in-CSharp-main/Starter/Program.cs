                ...
                if (ourAnimals[i, 1].Contains("dog"))
                {
                    ...
                    for (int j = 0; j < characteristics.Length; j++)
                    {
                        for (int k = 5; k > -1; k--)
                        {
                            ...
                            {
                                Console.Write($"\rsearching our dog {ourAnimals[i,3]} for {characteristics[j]} {icon} {characteristics.Length-1-j}   ");
                                Thread.Sleep(250);
                            }

                            Console.Write($"\r{new String(' ', Console.BufferWidth)}");
                        }
                        if (dogDescription.Contains(characteristics[j]))
                        {
                            // #3b update message to reflect term 
                            // #3c set a flag "this dog" is a match
                            Console.WriteLine($"Our dog {ourAnimals[i, 3]} matches your search for {characteristics[j]}");
                            currentMatch = true;
                            noMatchesDog = false;
                        }
                    }
